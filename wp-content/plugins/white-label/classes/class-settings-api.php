...
{ // phpcs:ignore
    ...
    public function callback_dashboard_widgets($args)
    {
        ...
        foreach ($args['options'] as $key => $label) {
            ...
            $html .= '<tr>';
            $html .= '<td class="wl-center">'.sprintf('<input type="checkbox" class="checkbox" id="wpuf-%1$s[%2$s][%3$s]" name="%1$s[%2$s][%3$s]" %4$s />', $args['section'], $args['id'], $key, checked($checked, 'on', false)).'</td>';
            $html .= '<td>'.sprintf('<label for="wpuf-%1$s[%2$s][%3$s]">%4$s</label><br>', $args['section'], $args['id'], $key, $label).'</td>';
        }
        $html .= '</tbody>';
        ...
        foreach ($args['options'] as $key => $label) {
            ...
            if ($submenus && is_array($submenus)) {
                foreach ($submenus as $slug => $name) {
                    ...
                    $html .= sprintf('<label class="wl-nested" for="wpuf-%1$s[%2$s][%3$s][%4$s][%5$s]">', $args['section'], $args['id'], 'children', $parent, $slug);
    ...
    public function callback_plugins($args)
    {
        ...
        $html .= '<thead>';
        $html .= '<tr>';
        $html .= '<td class="wl-center" style="width:8%;">'.__('Hide', 'white-label').'</td>';
        $html .= '<td style="width:auto;">'.__('Plugin', 'white-label').'</td>';
        $html .= '</tr>';
        $html .= '</thead>';
    ...
    public function callback_sidebar_menus($args)
    {
        ...
        $html .= '<tr>';
        $html .= '<td class="wl-center" style="width:8%;">'.__('Hide', 'white-label').'</td>';
        $html .= '<td style="width:auto;">'.__('Sidebar Menu', 'white-label').'</td>';
        $html .= '</tr>';
        $html .= '</thead>';
        ...
        foreach ($args['options'] as $key => $label) {
            ...
            $html .= '<td class="white_label-depth-0">'.$name.'</td>';
            $html .= '</tr>';
    ...
    public function callback_wysiwyg($args)
    {
        ...
        echo '<h4 class="white_label-name">'.$args['name'].'</h4>';
        echo $this->get_field_description($args);

        echo '<div class="white_label-wysiwyg" style="max-width: '.$size.';">';
