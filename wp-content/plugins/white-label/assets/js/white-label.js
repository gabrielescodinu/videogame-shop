document.addEventListener('DOMContentLoaded', function (e) {
    ...
    jQuery('.white_label-subheading').each(function (index, element) {
    ...
    const settings_tables = ['admin_welcome_panel_content', 'admin_remove_dashboard_widgets', 'admin_widget_content', 'admin_custom_dashboard_content', 'hidden_plugins', 'sidebar_menus', 'hidden_admin_bar_nodes_backend', 'hidden_admin_bar_nodes_frontend', 'admin_footer_credit'];
...
function white_label_login_preview(tab) {
  ...
  if (tab != '#white_label_login') {
      jQuery('.white_label-preview-box').hide();
  } else {
      jQuery('.white_label-preview-box').fadeIn(100);
  }
}
...
function white_label_upgrade_sidebar(tab) {
  ...
  if (tab == '#white_label_upgrade') {
      jQuery('.white_label-sidebar > .white_label-metabox:nth-of-type(2)').hide();
  } else {
      jQuery('.white_label-sidebar .white_label-metabox').fadeIn(100);
  }
}